...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/vincoin-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    VINCOIND_EXEC=vincoind.exe
fi
...
get_tag
popd
echo "var GUI_VINCOIN_VERSION = \"$TAGNAME\"" >> version.js

cd build
qmake ../vincoin-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy vincoind to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$VINCOIN_DIR/bin/$VINCOIND_EXEC $BIN_PATH
fi
