...
# Base environnement
        ...
        cd vincoin/utils/build_scripts
        docker build -f android32.Dockerfile -t vincoin-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t vincoin-gui-android .
        docker create -it --name vincoin-gui-android vincoin-gui-android bash

# Get the apk

        docker cp vincoin-gui-android:/opt/android/vincoin-core/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/vincoin-core/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
