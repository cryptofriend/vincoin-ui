...
{
    // prepare command line arguments and pass to vincoind
    ...
    qDebug() << "starting vincoind " + m_vincoind;
    ...
    // Start vincoind
    bool started = m_daemon->startDetached(m_vincoind, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM vincoind.exe");
#else
                QProcess::execute("pkill vincoind");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./vincoind status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd,bool testnet, QString &message) const
{
    ...
    p.start(m_vincoind, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to vincoind
#ifdef Q_OS_WIN
    m_vincoind = QApplication::applicationDirPath() + "/vincoind.exe";
#elif defined(Q_OS_UNIX)
    m_vincoind = QApplication::applicationDirPath() + "/vincoind";
#endif

    if (m_vincoind.length() == 0) {
